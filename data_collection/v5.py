...
def setup_yolo():
    ...
    try:
        ...
        try:
            model = YOLO('yolo11n.pt')  # Changed to YOLOv11
            print("YOLOv11 model loaded successfully")
            return model
        except Exception as e:
            print(f"Error loading YOLOv11 weights: {e}")
            print("Attempting to download weights...")
            try:
                model = YOLO('yolov11n.pt', task='detect')  # Changed to YOLOv11
                return model
            except Exception as e:
...
class ImageCapture:
    ...
    def get_color_name(self, blueprint):
        """Get all available color options for the vehicle"""
        if blueprint.has_attribute('color'):
            try:
                color_attr = blueprint.get_attribute('color')
                if color_attr.is_modifiable and color_attr.recommended_values:
                    # Get all available colors and select one
                    color_options = color_attr.recommended_values
                    selected_color = random.choice(color_options)
                    blueprint.set_attribute('color', selected_color)
                    return selected_color.split('/')[-1].split('.')[0].replace(' ', '_')
            except Exception as e:
    ...
    def detect_and_crop(self, image):
        """Process image with YOLOv11 and prepare for ResNet with proper colors"""
        try:
            ...
            # YOLOv11 detection with retries
            ...
            if results:
                for result in results:
                    for box in result.boxes:
                        if int(box.cls) in [2, 5, 7] and box.conf > YOLO_CONFIDENCE:  # 2: car, 5: bus, 7: truck
            ...
            if best_box:
                ...
                # Center crop fallback with padding
            ...
            return self.resize_transform(padded), bool(best_box)
        except Exception as e:
            print(f"Error in detect_and_crop: {e}")
            # Return blank image if processing fails
            blank = Image.new('RGB', RESNET_IMAGE_SIZE, PADDING_COLOR)
            return blank, False
