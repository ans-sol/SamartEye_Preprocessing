...
ASPECT_RATIO = RESNET_IMAGE_SIZE[0] / RESNET_IMAGE_SIZE[1]  # Target aspect ratio
PADDING_COLOR = (114, 114, 114)  # Gray padding color (YOLO standard)
...
def setup_yolo():
    ...
    try:
        ...
        try:
            model = YOLO('yolo11n.pt')  # Using stable YOLOv8
...
def get_all_vehicle_colors(world, blueprint):
    """Get all available color options for a vehicle blueprint"""
    if not blueprint.has_attribute('color'):
        return [None]  # No color attribute
    
    color_attr = blueprint.get_attribute('color')
    if not color_attr.is_modifiable or not color_attr.recommended_values:
        return [None]  # No modifiable colors
    
    return color_attr.recommended_values
...
class ImageCapture:
    def __init__(self, output_root, vehicle_blueprint):
        ...
        # Remove color subfolder - now all colors go in vehicle type folder
        self.output_dir = os.path.join(output_root, self.vehicle_type)
    ...
    def get_color_name(self, blueprint):
        if blueprint.has_attribute('color'):
            try:
                color = blueprint.get_attribute('color').as_color()
                # Return simplified color name for filename
                return f"{color.r}_{color.g}_{color.b}"  # e.g., "187_0_0"
            except:
                pass
        return "default"
    ...
    def add_padding(self, image, target_ratio=ASPECT_RATIO):
        """Add padding to maintain aspect ratio without stretching"""
        ...
        if current_ratio > target_ratio:
            ...
            padding = (0, (new_height - height) // 2, 0, (new_height - height) - (new_height - height) // 2)
        else:
            # Image is taller than target - add padding left and right
            new_width = int(height * target_ratio)
            padding = ((new_width - width) // 2, 0, (new_width - width) - (new_width - width) // 2, 0)
            
        return ImageOps.expand(image, border=padding, fill=PADDING_COLOR)

    def detect_and_crop(self, image):
        """Process image with YOLO and prepare for ResNet with proper colors"""
        try:
            # Convert from CARLA's raw BGRA format to RGB
            array = np.frombuffer(image.raw_data, dtype=np.dtype("uint8"))
            array = np.reshape(array, (image.height, image.width, 4))
            
            # Convert BGRA to RGB (drop alpha channel and convert BGR to RGB)
            rgb_array = cv2.cvtColor(array[:, :, :3], cv2.COLOR_BGR2RGB)
            pil_image = Image.fromarray(rgb_array)
            
            # YOLO detection with retries
            results = self.safe_detect(rgb_array)
            best_box = None
            
            if results:
                for result in results:
                    for box in result.boxes:
                        if int(box.cls) in [2, 3, 5, 7] and box.conf > YOLO_CONFIDENCE:  # 2: car, 5: bus, 7: truck
            ...
            if not best_box:
                return None, False  # Skip images without detection
            
            x1, y1, x2, y2 = best_box
            # Add 20% margin but keep within bounds
            ...
            padded = self.add_padding(cropped)
            
            return self.resize_transform(padded), True
        except Exception as e:
    ...
    def save_image(self, image):
        with self.lock:
            try:
                ...
                if not detected:
                    print("⏭️ Skipping (no vehicle detected)")
                    self.ready.set()
                    return

                # Include color in filename instead of folder structure
                counter_str = f"{self.image_count:04d}"
                filename = os.path.join(
                    self.output_dir,
                    # Now includes color in filename: "vehicle_nissan_micra_187_0_0_0001.png"
                    f"{self.vehicle_type}_{self.color_name}_{counter_str}.png"
                )
...
def main():
    ...
    try:
        ...
        for blueprint in vehicle_blueprints:
            ...
            # Get all available colors for this vehicle
            color_options = get_all_vehicle_colors(world, blueprint)
            if not color_options:
                color_options = [None]  # Default if no colors available
            
            for color in color_options:
                try:
                    # Create a fresh copy of the blueprint for each color
                    current_bp = world.get_blueprint_library().find(blueprint.id)
                    
                    # Set color if available
                    if color is not None:
                        current_bp.set_attribute('color', color)
                    
                    transform = carla.Transform(base_location, base_rotation)
                    vehicle = spawn_vehicle_with_retry(world, current_bp, transform)
                    ...
                    image_capture = ImageCapture('dataset', current_bp)
                    ...
                    print(f"Error processing vehicle {blueprint.id} with color {color}: {e}")
