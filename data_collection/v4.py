...
class ImageCapture:
    ...
    def get_color_name(self, blueprint):
        """Get color name from vehicle blueprint with all available colors"""
        if blueprint.has_attribute('color'):
            try:
                color = blueprint.get_attribute('color')
                if color.is_modifiable:
                    # Get all available color options
                    color_options = color.recommended_values
                    if color_options:
                        # Randomly select one color from available options
    ...
    def detect_and_crop(self, image):
        """Process image with YOLOv10 and prepare for ResNet with proper colors"""
        try:
            ...
            results = self.safe_detect(rgb_array)  # Note: Using RGB array for detection
            best_box = None
...
def main():
    ...
    parser.add_argument('--num-colors', type=int, default=3, help='Number of color variations per vehicle')
    ...
    try:
        ...
        for blueprint in vehicle_blueprints:
            ...
            # Get all available colors for this blueprint
            color_options = []
            if blueprint.has_attribute('color'):
                color_attr = blueprint.get_attribute('color')
                if color_attr.is_modifiable:
                    color_options = color_attr.recommended_values
            
            # If no color options, just use default once
            if not color_options:
                color_options = [None]
            else:
                # Select up to args.num_colors random colors
                color_options = random.sample(color_options, min(args.num_colors, len(color_options)))
