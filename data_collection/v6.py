...
class ImageCapture:
    def __init__(self, output_root, vehicle_blueprint):
        ...
        self.output_dir = os.path.join(
            output_root,
            self.vehicle_type,
            self.color_name.lower()  # Ensure lowercase for consistency
        )
    ...
    def get_color_name(self, blueprint):
        if blueprint.has_attribute('color'):
            try:
                ...
                return f"{color.r},{color.g},{color.b}"  # e.g., "187,0,0"
            except:
                pass
        return "default"
    ...
    def save_image(self, image):
        with self.lock:
            try:
                ...
                # Ensure 4-digit zero-padded counter (0001, 0002, ...)
                counter_str = f"{self.image_count:04d}"
                filename = os.path.join(
                    self.output_dir,
                    f"{self.vehicle_type}_{counter_str}.png"  # e.g., "vehicle_nissan_micra_0001.png"
                )
...
def main():
    ...
    try:
        ...
        target_vehicles = [
            'vehicle.nissan.micra',  # Only one vehicle for testing
        ]
